...
#error "No cryptographic backend selected"
    ...
    EDHOC_CHECK_RET(edhoc_msg2_decode(ctx, msg2_buf, msg2_len));
    ...
    if ((ret = crypt_hash_update(&transcript_ctx, out, ret)) != EDHOC_SUCCESS) {
        goto exit;
    }

    // generate data_2
    if ((ret = edhoc_data2_encode(
            ctx->correlation,
            ctx->session.cidi,
            ctx->session.cidi_len,
            ctx->session.cidr,
    ...
    if ((ret = crypt_hash_finish(&transcript_ctx, ctx->th_2)) != EDHOC_SUCCESS) {
        goto exit;
    }
    ...
    if ((ret = crypt_edhoc_kdf(aead, ctx->prk_2e, ctx->th_2, "K_2e", k2e_buf, ctx->ct_or_pld_2_len)) < EDHOC_SUCCESS)
        goto exit;
    ...
    if ((ret = crypt_hash_update(&transcript_ctx, out, data3_len)) != EDHOC_SUCCESS) {
        goto exit;
    }
    ...
    if ((ret = edhoc_p2e_decode(ctx->ct_or_pld_2, ctx->ct_or_pld_2_len)) != EDHOC_SUCCESS)
        goto exit;

    EDHOC_CHECK_RET(crypt_edhoc_kdf(aead, ctx->prk_4x3m, ctx->th_3, "K_3m", k3m_or_k3ae_buf, key_len));
    EDHOC_CHECK_RET(crypt_edhoc_kdf(aead, ctx->prk_4x3m, ctx->th_3, "IV_3m", iv3m_or_iv3ae_buf, iv_len));

    if ((ret = compute_signature_or_mac23(ctx,
                                          k3m_or_k3ae_buf,
                                          iv3m_or_iv3ae_buf,
                                          ctx->conf->ad3,
                                          ctx->th_3,
                                          signature_or_mac3_buf)) != EDHOC_SUCCESS) {
        goto exit;
    }
    ...
    if ((ret = edhoc_msg3_encode(out, data3_len, ctx->ct_or_pld_3, ctx->ct_or_pld_3_len, out, olen)) < EDHOC_SUCCESS) {
        goto exit;
    }
...
ssize_t edhoc_create_msg2(edhoc_ctx_t *ctx,
                          ...
                          uint8_t *out,
                          size_t olen) {
    ssize_t ret, size, key_len, iv_len;
    cose_curve_t crv;
    cose_algo_t aead;
...
#error "No cryptographic backend selected"
    ...
    EDHOC_CHECK_RET(crypt_compute_prk2e(ctx->secret, NULL, 0, ctx->prk_2e));
    EDHOC_CHECK_RET(crypt_compute_prk3e2m(ctx->conf->role, *ctx->method, ctx->prk_2e, ctx->secret, ctx->prk_3e2m));
    EDHOC_CHECK_RET(crypt_edhoc_kdf(aead, ctx->prk_3e2m, ctx->th_2, "K_2m", k2m_buf, key_len));
    EDHOC_CHECK_RET(crypt_edhoc_kdf(aead, ctx->prk_3e2m, ctx->th_2, "IV_2m", iv2m_buf, iv_len));
    ...
    if ((ret = compute_signature_or_mac23(ctx,
                                          ...
                                          ctx->conf->ad2,
                                          ctx->th_2,
                                          signature_or_mac2_buf)) != EDHOC_SUCCESS)
        goto exit;
    ...
    if ((ret = crypt_edhoc_kdf(aead, ctx->prk_2e, ctx->th_2, "K_2e", k2e_buf, p2e_len)) < EDHOC_SUCCESS) {
        goto exit;
    }
    ...
    if ((size = edhoc_msg2_encode(out, data2_len, ctx->ct_or_pld_2, ctx->ct_or_pld_2_len, out, olen)) < EDHOC_SUCCESS) {
        ret = size;
        goto exit;
    }
