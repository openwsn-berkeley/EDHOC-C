...
const uint8_t cborAuthKey[] = {0xa4, 0x01, 0x01, 0x20, 0x06, 0x23, 0x58, 0x20, 0xdf, 0x69, 0x27, 0x4d, 0x71, 0x32, 0x96,
...
const uint8_t certificate[] = {0x58, 0x6e, 0x47, 0x62, 0x4d, 0xc9, 0xcd, 0xc6, 0x82, 0x4b, 0x2a, 0x4c, 0x52, 0xe9, 0x5e,
...
const uint8_t cborEphKey[] = {0xa5, 0x01, 0x01, 0x20, 0x04, 0x21, 0x58, 0x20, 0x71, 0xa3, 0xd5, 0x99, 0xc2, 0x1d, 0xa1,
...
const uint8_t credId[] = {0xa1, 0x18, 0x22, 0x82, 0x2e, 0x48, 0xfc, 0x79, 0x99, 0x0f, 0x24, 0x31, 0xa3, 0xf5};
...
int main(void) {
    ...
    if (edhoc_cred_load_cbor_cert(&cert, certificate, sizeof(certificate)) != 0)
        return -1;

    DEBUG("Load private authentication key...\n");
    edhoc_conf_load_authkey(&conf, cborAuthKey, sizeof(cborAuthKey));
    ...
    if (edhoc_conf_load_cred_id(&conf, credId, CRED_ID_TYPE_X5T, sizeof(credId)) != 0)
        return -1;
